...
export function DailySentChart() {
  const data = [
    { day: "Aug 22", newLead: 0, followUp: 0 },
    { day: "Aug 24", newLead: 0, followUp: 0 },
    { day: "Aug 26", newLead: 0, followUp: 1 },
    { day: "Aug 28", newLead: 0, followUp: 0 },
    { day: "Aug 30", newLead: 1, followUp: 2 },
    { day: "Sep 1", newLead: 0, followUp: 0 },
    { day: "Sep 3", newLead: 0, followUp: 5 },
    { day: "Sep 4", newLead: 0, followUp: 0 },
  ];

  return (
    <div className="h-[220px] w-full">
      <ResponsiveContainer className="[&_.recharts-surface]:outline-none" height="100%" width="100%">
        <RechartsBarChart data={data} margin={{ top: 12, right: 2, left: -12, bottom: -4 }}>
          <CartesianGrid stroke="var(--separator)" strokeDasharray="0" vertical={false} />
          <XAxis
            axisLine={false}
            dataKey="day"
          ...
          <Tooltip
            ...
            cursor={{ fill: "color-mix(in oklch, var(--foreground) 5%, transparent)" }}
          />
          <Bar dataKey="newLead" fill="var(--success)" name="New Lead" radius={[4, 4, 0, 0]} stackId="sent" />
          <Bar dataKey="followUp" fill="var(--accent)" name="Follow-up" radius={[4, 4, 0, 0]} stackId="sent" />
        </RechartsBarChart>
      </ResponsiveContainer>
    </div>
  );
...
export function SendVolumeChart({ data }: { data: ReputationTrendPoint[] }) {
  const chartData = data.map((point) => ({
    ...point,
    label: new Date(point.date).toLocaleDateString(undefined, { month: "short", day: "numeric" }),
  }));
  const maxVolume = Math.max(10, ...chartData.map((point) => point.sentVolume));

  return (
    <div className="h-[136px] w-full">
      <ResponsiveContainer className="[&_.recharts-surface]:outline-none" height="100%" width="100%">
        <RechartsBarChart accessibilityLayer data={chartData} margin={{ top: 12, right: 2, left: -12, bottom: -4 }}>
          <CartesianGrid stroke="var(--separator)" strokeDasharray="0" vertical={false} />
          <XAxis
            axisLine={false}
            dataKey="label"
          ...
          <Tooltip
            ...
            cursor={{ fill: "color-mix(in oklch, var(--foreground) 5%, transparent)" }}
          />
          <Bar animationDuration={450} barSize={14} dataKey="sentVolume" fill="var(--accent)" radius={[7, 7, 7, 7]} />
        </RechartsBarChart>
      </ResponsiveContainer>
    </div>
  );
...
export function ReputationTrendChart({ data }: { data: ReputationTrendPoint[] }) {
  const chartData = data.map((point) => ({
    ...point,
    label: new Date(point.date).toLocaleDateString(undefined, { month: "short", day: "numeric" }),
  }));
  const rawMax = Math.max(0.1, ...chartData.map((point) => Math.max(point.bounceRate, point.complaintRate)));
  const niceMax = Math.max(1, Math.ceil(rawMax * 1.25));
  ...
  return (
    <div className="h-[136px] w-full">
      <ResponsiveContainer className="[&_.recharts-surface]:outline-none" height="100%" width="100%">
        <RechartsLineChart data={chartData} margin={{ top: 10, right: 2, left: -10, bottom: -4 }}>
          <CartesianGrid stroke="var(--separator)" strokeDasharray="0" vertical={false} />
          <XAxis
            axisLine={false}
            dataKey="label"
          ...
          <Tooltip
            ...
            cursor={false}
          />
          <Line animationDuration={650} dataKey="bounceRate" dot={false} name="Bounce rate" stroke="var(--accent)" strokeWidth={2} type="linear" />
          <Line animationDuration={650} dataKey="complaintRate" dot={false} name="Complaint rate" stroke="color-mix(in oklch, var(--accent) 58%, white)" strokeWidth={2} type="linear" />
        </RechartsLineChart>
      </ResponsiveContainer>
    </div>
  );
